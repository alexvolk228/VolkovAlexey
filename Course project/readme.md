...
С применением системы экономия времени составит 22,5 часа в день на отдел менеджеров из 5-х человек при количестве заказов в день равным 100, это позволит иметь в штате меньшее количество сотрудников, тем самым снижая затраты на зарплаты. Также снижается вероятность ошибок, т.к. все расчеты делает система.
...
Так как в день на одного менеджера экономия составляет 22,5 часов, то в день отдел из 5-ти менеджеров экономит 112,5 часов. Работают менеджеры по 5 дней в неделю. В месяц экономия составляет 2250 ч/час. Значит внедрение системы будет иметь точку безубыточности примерно через 8 месяцев после даты начала использования.

В результате расчета сложности разработки методом FPA IFPUG было определено 3264 строк кода. На данный момент система разработана на 50% и имеет 1632 строки кода. Снижение строк кода возможно только в минимальном количестве. Трудозатраты были снижены за счет упрощения документирования и с использованием предыдущих наработок. Определен риск срыва сроков. При наступлении риска срыва сроков разработки будут сокращены требования для текущей версии системы, тем самым снизив трудозатраты на разработку и количество строк кода.
