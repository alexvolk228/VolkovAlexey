...
**Конфигурация технических средств:** сервер (Apache/2.4.46, PHP/7.4.12), осуществляющий хранение и обработку данных. ПК для работы с веб-приложением. Операционная система Windows, Linux или Mac OS.  
...
* Налажена связь между интернет-магазином и CRM-системой

**4.6 Постусловия (post-conditions):**

* Заказ в CRM-системе с назначенным менеджером и статусом "Передан на производтво"

**4.7 Основной поток выполнения (main flow)**:

| Участник  | Действие (activity)  | Ожидаемый результат |
|---|---|---|
| Менеджер | Назначение менеджера на заказ | Заказ с назначенным менеджером |
| Менеджер | Смена статуса заказа | Заказ со статусом "Передан на производство" |

**4.8 Исключения (exceptions):**

| Условие (риск) | Последствия | Реакция |
|---|---|---|
| Ошибка при получении заказа из интернет-магазина| Невожможность начать обрабатывать заказ | Запрос на повторное копирование заказа |

**4.9 Альтернативы (alternates):**

| Участник  | Действие (activity)  | Ожидаемый результат |
|---|---|---|
| Менеджер | Приём заказа по телефону | Новый способ получения заказов |

**4.10 Временные параметры:**

* **Триггер (событие, стартующее прецедент):** поступление заказа из интернет-магазина. 

* **Номинальная частота повторения прецедента:** 100 раз в сутки

* **Продолжительность прецедента:** 1,5 минуты
...
После внедрения системы управления взаимоотношениями с клиентами, на обработку одного заказа будет тратиться 30 секунд на получение заказа интернет-магазина и проверку его корректности + 1 минута на передачу заказа на производство. Итого - 1,5 минуты. 

**10.3 Расчет потенциального эффекта от автоматизации:**

Менеджер тратит 10 мин на получение одного заказа и 5 минут на передачу заказа на производство.
В день в среднем поступает 100 заказов. Следовательно, одному менеджеру нужно иметь рабочую смену в 25 часов для обработки всех заказов, когда рабочая смена должна быть 8 часов. Для успешной обработки такого количества заказов потребуется, как минимум, 4 сотрудника. А один менеджер за смену (8 часов) может обработать = 32 заказа. 

После внедрения CRM-системы экономия составит 13,5 минут с одного заказа, или 22,5 часа в день на одного менеджера. С автоматизированной системой менеджер сможет обработать за смену = 320 заказов, что в 10 раз больше. А при суточном потоке в 100 заказов, с данной работой справится всего 1 менеджер.
...
Исходя из расчётов трудозатрат, полученных методом COCOMO II делаем вывод, что система является экономически выгодной (с учетом того, что программисты имеют низкие навыки) и может быть реализована за 9 месяцев командой из 4-х человек.
...
В результате расчета сложности разработки методом FPA IFPUG было получено 3264 строк кода. На данный момент было затрачено 55 ч/час, система разработана на 50% и имеет 1632 строки кода. Снижение строк кода возможно только в минимальном количестве. Трудозатраты были снижены за счет упрощения документирования и использования предыдущих наработок. Определен риск срыва сроков. При срыве сроков разработки будут сокращены требования для текущей версии системы, тем самым снизятся трудозатраты на разработку и количество строк кода.
